...
(() => {
  // Presence marker so the dashboard can tell the posting preview is unblocked (the
  // declarativeNetRequest rules in background.js). Only on the dashboard's own host, so
  // no other site can fingerprint the extension, and only while the popup's "Posting
  // preview" toggle is on, so the feed falls back to Open / cached when it is off.
  try {
    chrome.storage.sync.get(['serverUrl', 'previewUnblock'], (s) => {
      let host = 'localhost';
      try { host = new URL(s.serverUrl || 'http://localhost').hostname.toLowerCase(); } catch (_) {}
      if (location.hostname.toLowerCase() !== host || s.previewUnblock === false) return;
      document.documentElement.setAttribute('data-jn-ext', chrome.runtime.getManifest().version || '1');
    });
  } catch (_) {}
