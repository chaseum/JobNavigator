...
DEFAULT_SETTINGS = {
    ...
    "knowledge_embedding_model": ("embeddinggemma", "Ollama embedding model for Knowledge Bank chunks and retrieval (Advanced/Developer)"),
...
def run_migrations(db):
    ...
    migrations = [
        ...
        "ALTER TABLE jobs ADD COLUMN IF NOT EXISTS externally_added_at TIMESTAMPTZ",
        "CREATE INDEX IF NOT EXISTS ix_jobs_externally_added_at ON jobs(externally_added_at)",
        "ALTER TABLE resume_sources ADD COLUMN IF NOT EXISTS report JSONB",
        "ALTER TABLE resume_sources ADD COLUMN IF NOT EXISTS mime_type VARCHAR(120) DEFAULT 'application/pdf'",
        "ALTER TABLE resume_sources ADD COLUMN IF NOT EXISTS processed_at TIMESTAMPTZ",
        "ALTER TABLE candidate_facts ADD COLUMN IF NOT EXISTS embedding JSONB",
        "ALTER TABLE candidate_facts ADD COLUMN IF NOT EXISTS embedding_text_sha256 VARCHAR(64)",
        "CREATE INDEX IF NOT EXISTS ix_candidate_facts_embedding_text_sha256 ON candidate_facts(embedding_text_sha256)",
        """CREATE TABLE IF NOT EXISTS knowledge_chunks (
            id SERIAL PRIMARY KEY,
            document_id INTEGER NOT NULL REFERENCES resume_sources(id) ON DELETE CASCADE,
        ...
        # Legacy hand-added rows used source as their only origin marker.
        "UPDATE jobs SET externally_added_at = COALESCE(externally_added_at, NOW()) WHERE source IN ('extension', 'manual')",
