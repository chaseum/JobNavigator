...
### Added
- **Résumé Library and Career Evidence** (`/profile`): onboarding is now "upload the résumés you already have", not "fill in a dossier". Drop in several historical résumé PDFs at once; the text is extracted in the request and the structuring runs as one background job. Each document is a `ResumeSource`, and `CandidateFactSource` records which uploads support each canonical fact, so every fact shows *Used in: resume-swe-2025.pdf*. Reconciliation is deterministic and never asks a model to decide: a compatible duplicate attaches its source and fills only empty fields, a newer résumé's extra bullets join the same canonical role, and a field two résumés state differently (a graduation date, a GPA) becomes a **conflict** you resolve yourself — the stored value is never overwritten. Entries are matched on normalised employer, title, school, degree, project and skill names rather than raw text, and an achievement is only ever compared with its own parent's. Imported facts stay unverified, with select-all bulk confirmation; **Add Fact** stays for whatever your résumés never said.
- **Role-family base résumés**: Software Engineering, Product / Technical PM and Data / ML (plus any you add in Settings › Advanced). A family base is a *view* of the same verified Career Evidence — which experience, projects and skills lead — not a second copy of the truth and never new wording. A job is classified into a family deterministically from its structured analysis, with a reason and an override on the job's Workspace, and its tailored résumé derives from that family's base. Classification is deliberately no part of Candidate Fit.
- **Jake's Resume template** (`backend/resume/templates/jakes`): Jake Gutierrez's MIT-licensed layout, vendored verbatim from [jakegut/resume](https://github.com/jakegut/resume) with its license and `SOURCE.md` (author, repository, Overleaf link, revision), and parameterised through the existing deterministic Jinja/LaTeX renderer. It is the default generated layout; an explicit template setting is kept. A regression test renders a realistic résumé, compiles it and asserts the headings, contact, experience and skills survive PDF text extraction.
- **Candidate fact database and Profile** (`/profile`): employment, internships, projects, research, skills with usage evidence, education, certifications, achievements, publications and links as typed rows with provenance ids (`experience_7`, `achievement_12`). Résumé import (PDF, base résumé or Persona) creates unverified facts that nothing uses until you verify them.
- **Structured LLM output**: `call_structured` validates every model reply against a pydantic schema: Ollama `format` at temperature 0, OpenAI `json_schema`, a forced Claude tool call. It has no silent fallback to another provider.
- **Role Match** (a job's Workspace, `/jobs/:id`): requirement extraction, evidence mapping to verified facts, a deterministic 0–100 score with visible components and configurable weights, a MATCHED / PARTIAL / MISSING / UNKNOWN matrix, and gap analysis with add-context. Uncited or dangling citations earn nothing; work authorization comes only from the user's answers.
- **LaTeX résumé pipeline**: a résumé plan from facts, bullet rewrites that cite `source_fact_ids`, machine checks plus a second model audit (unsupported claims block the PDF), a fixed template rendered and compiled locally, and Parser Health on the extracted PDF text. Includes a claim-by-claim diff review (accept, reject, regenerate, edit), immutable accepted versions, and output folders under `generated/`.
- **Overleaf**: `.tex`, PDF and ZIP export; optional Git-remote push and pull, with the token taken from the environment only.
- **Extension**: Role Match in the popup after Save Job; on application pages of saved jobs it uploads the accepted résumé, reuses Answer Bank answers, adds a label-only semantic field-mapping fallback, outlines fields needing review, offers "Save answer for future applications", and records the application. It never submits.
- **Answer Bank** (`/answer-bank`): equivalent wordings share one entry (aliases, intent, verified, last used). Protected questions (authorization, sponsorship, EEO, salary, attestations) are refused by the drafting endpoints unless the user saved a verified answer.
- **Dashboard** (`/dashboard`) and Settings › Copilot (weights, résumé template and policies, autofill fallback, Overleaf, and a per-feature *Where your data goes*).

### Changed
- **Each keyword job board runs in its own killable process.** LinkedIn, Indeed, Google and ZipRecruiter were one `scrape_jobs()` call wrapped in a thread, which gave no kill boundary — and `python-jobspy` 1.1.82 can hang paginating Indeed, stranding the whole search. Every board now runs as its own subprocess under a timeout (Settings › Advanced › Job board timeout, default 150 s); a board that hangs, crashes or 403s costs only itself and the rows the others returned are still stored.
- **Scraping no longer depends on the legacy CV scorer.** The post-scrape scoring sweep is contained on every path, so a scorer that raises or has no provider configured can no longer fail a successful search.
- Parser Health folds typographic quotes before comparing, so TeX rendering a typed `'` as `’` stops reading as a lost bullet.
- The backend image adds `texlive-latex-extra` and `texlive-fonts-extra` for the packages Jake's template loads, keeping `glyphtounicode` / `pdfgentounicode` so generated text stays extractable.
- Default LLM provider is **Ollama** on fresh installs, with no model assumed. `ollama_base_url` / `OLLAMA_BASE_URL` configure the endpoint, and Docker adds `host.docker.internal`.
- Applications have twelve stages (Saved → Analyzed → Résumé drafted → Ready to apply → Applied → OA → Recruiter screen → Interview → Final → Offer → Rejected / Withdrawn) and record the résumé version, the match score at application time, and the answers used. "Save as applied" on an application the extension prepared moves it to Applied instead of refusing a duplicate, and it finds the job by URL when company or title differ.
- 
